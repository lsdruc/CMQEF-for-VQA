...
def train(model, train_loader,num_epochs, output):
    ...
    logger = utils.Logger(os.path.join(output, 'cpv2trainBDSR.log'))
    slr = 0.67
    cost_best_last=100.0
    cost_best_con = 100.0
    lr=0.002
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b) in tqdm(enumerate(train_loader), ncols=100,desc="Epoch %d" % (epoch+1), total=len(train_loader)):
            v = Variable(v).cuda()
            b = Variable(b).cuda()
            q = Variable(q).cuda()
            a = Variable(a).cuda()
        ...
        if optim.param_groups[0]['lr'] < numpy.float32(0.0001):

            if slr < numpy.float32(0.95):
                newlr = lr * numpy.float32(0.75)
                slr = slr / numpy.float32(0.9)
                optim.param_groups[0]['lr']=newlr
            else:
                slr = numpy.float32(0.95)
                newlr = lr * numpy.float32(0.8)
                optim.param_groups[0]['lr']=newlr
        else:
            cost_best_con = total_loss
            if numpy.float32(cost_best_con - cost_best_last) + numpy.float32(0.03) > numpy.float32(0.017):
                optim.param_groups[0]['lr'] = optim.param_groups[0]['lr'] * slr
            elif numpy.float32(cost_best_con - cost_best_last) + numpy.float32(0.03) < 0:
                cost_best_last = cost_best_con
                optim.param_groups[0]['lr'] = optim.param_groups[0]['lr'] * numpy.float32(1.0 + slr * 0.1)

def evaluate(model, dataloader,output,model_name):

    logging.basicConfig(level=logging.DEBUG, filename='cpv2testBDSR.log')
